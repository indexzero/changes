...
// ####   @options.includeDocs    {Boolean} should include complete docs or not.
...
var Changes = module.exports = function (options) {
  ...
  this.includeDocs  = options.includeDocs !== undefined ? options.includeDocs : true;
  this.retry        = {
    enabled:  true,
    timeout:  0,
    attempts: 0,
...
Changes.prototype.listen = function (updateSeq, callback) {
  ...
  // first update.
  //
  var feedUrl = this.url + '/_changes?feed=continuous&include_docs=' + this.includeDocs + '&since=' + (updateSeq || 0),
    ...
    changes.on('data', function (data) {
      ...
      for (var i = 0; i < parts.length - 1; i++) {
        ...
        try {
          ...
          if (!row) return;
          updateSeq = row.last_seq || row.seq || updateSeq;
        }
        catch (e) {
  ...
  async.forEach(
    ...
    function queryOne(view, next) {
      ...
      request({
        ...
        //
        updateSeq = json.update_seq || updateSeq || 0;
